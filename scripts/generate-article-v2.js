...
/**
 * Generate Article (v2 - Optimized)
 * Supports multiple AI providers with fallback mode
 * Improved error handling, timeouts, and retry logic
 */
...
/**
 * Get prompt file for content type
 */
...
async function callGemini(prompt, topicData) {
  ...
  try {
    ...
    try {
      validateApiResponse(data, ['candidates']);
      ...
      console.error('Gemini response validation failed:', error.message);
      return null;
    }
  } catch (error) {
...
async function callOpenAI(prompt, topicData) {
  ...
  const model = process.env.AI_MODEL || OPENAI.DEFAULT_MODEL;
  ...
  try {
    ...
    // Validate response structure
    try {
      validateApiResponse(data, ['choices']);
...
function generateFallbackContent(topicData) {
  ...
  const modelUsed = process.env.AI_API_KEY ? (process.env.AI_MODEL || 'openai-compatible') : 'fallback';
...
*Generated by autonomousBLOG using ${modelUsed} on ${now.toUTCString()}*
`;
}
...
async function callAI(prompt, topicData) {
  ...
  if (process.env.GEMINI_API_KEY) {
    ...
    console.log('⚠️  Gemini API failed, trying next provider...');
  }

  // Try OpenAI-compatible API
  if (process.env.AI_API_KEY) {
    console.log('📡 Attempting OpenAI-compatible API...');
    const result = await callOpenAI(prompt, topicData);
    if (result) {
      console.log('✅ OpenAI-compatible API succeeded');
      return result;
    }
    console.log('⚠️  OpenAI-compatible API failed...');
  }

  // Fall back to template-based generation
  console.log('💡 Using fallback content generation...');
  console.log('📝 Tip: Set GEMINI_API_KEY environment variable for better quality (free at https://aistudio.google.com)');
  return null;
}
...
async function generateArticle() {
  ...
  let topicData;
  try {
    topicData = JSON.parse(fs.readFileSync(TOPIC_FILE, 'utf8'));
  } catch (error) {
    throw new Error(`Failed to parse topic file: ${error.message}`);
  }

  if (!topicData.topic) {
    throw new Error('Invalid topic data: missing topic field');
  }
  ...
  // Ensure content has frontmatter and model attribution
  if (!content.includes('---')) {
    content = `---
...
*Generated by autonomousBLOG using ${modelUsed} on ${now.toUTCString()}*
`;
  } else if (!content.includes('Generated by autonomousBLOG')) {
    // Add model attribution to existing frontmatter content
    content = content.replace(
      /(\*Generated by autonomousBLOG on [\s\S]*?\*)$/,
      `*Generated by autonomousBLOG using ${modelUsed} on ${now.toUTCString()}*`
    );
  }
  ...
  try {
    ...
    throw new Error(`Failed to write article file: ${error.message}`);
  }
  ...
  try {
    ...
    console.warn('Failed to clean up topic file:', error.message);
  }
