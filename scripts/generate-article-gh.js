...
/**
 * Generate Article with GitHub Models
 * Uses gh cli for inference
 */

const fs = require('fs');
const path = require('path');
const GhInference = require('./gh-inference');
...
/**
 * Call GitHub Models API for content generation
 */
async function callGhModels(prompt, topicData) {
  const inference = new GhInference(GH_MODEL);
  inference.setupGitIdentity();
  
  const result = await inference.generate(prompt, {
    temperature: 0.7,
    maxTokens: 4096,
    verbose: true,
    systemPrompt: "You are a professional blog writer specialized in deep technical and philosophical articles."
  });

  if (!result.success) {
    throw new Error(result.error || 'Failed to generate content with GitHub Models');
  }
...
async function generateArticle() {
  ...
  console.log('🤖 Generating with GitHub Models...');
  let content = await callGhModels(prompt, topicData);
